...
//    @Query("SELECT job_order_id, job_order_number, customer_id, jos.svc_machine_type as machine_type, cu.name as customer_name FROM job_orders jo JOIN customers cu ON jo.customer_id = cu.id JOIN job_order_services jos ON job_order_id = jo.id LEFT JOIN services w ON w.id = service_id WHERE jos.svc_machine_type = :machineType AND jos.quantity > used GROUP BY customer_name")
    @Query("SELECT SUM(quantity - used) as available, customer_id, address, crn, jos.svc_machine_type as machine_type, cu.name as customer_name, MAX(jo.created_at) AS latest_job_order FROM job_orders jo JOIN customers cu ON jo.customer_id = cu.id JOIN job_order_services jos ON job_order_id = jo.id LEFT JOIN services w ON w.id = service_id WHERE jos.svc_machine_type = :machineType AND jos.quantity > used AND jos.deleted_at IS NULL GROUP BY customer_name ORDER BY latest_job_order ASC")
