...
abstract class DaoJobOrderService : BaseDao<EntityJobOrderService> {
    @Query("SELECT service_name, COUNT(*) as count, svc_machine_type, svc_wash_type FROM job_order_services WHERE strftime('%Y-%m-%d', created_at / 1000, 'unixepoch') = :dateFrom OR ( :dateTo IS NOT NULL AND strftime('%Y-%m-%d', created_at / 1000, 'unixepoch') BETWEEN :dateFrom AND :dateTo ) GROUP BY service_name, svc_wash_type, svc_machine_type")
