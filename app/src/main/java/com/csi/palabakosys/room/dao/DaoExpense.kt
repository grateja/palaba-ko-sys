...
abstract class DaoExpense : BaseDao<EntityExpense> {
    ...
    @Query("SELECT tag, COUNT(*) as count, SUM(amount) as sum FROM expenses WHERE strftime('%Y-%m-%d', created_at / 1000, 'unixepoch') = :dateFrom OR ( :dateTo IS NOT NULL AND strftime('%Y-%m-%d', created_at / 1000, 'unixepoch') BETWEEN :dateFrom AND :dateTo ) GROUP BY tag")
