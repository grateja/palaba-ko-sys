...
abstract class DaoCustomer : BaseDao<EntityCustomer> {
    ...
    @Query("SELECT * FROM customers WHERE name LIKE '%' || :keyword || '%' AND deleted_at IS NULL ORDER BY name")
    ...
    @Query("SELECT id, crn, name, address FROM customers WHERE name LIKE '%' || :keyword || '%' OR crn like '%' || :keyword || '%' AND deleted_at IS NULL ORDER BY name")
